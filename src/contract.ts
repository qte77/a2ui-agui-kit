...
/*
 * Validated contracts for the A2UI wire format that crosses agent → UI, plus a generic
 * "recording" shape for pre-baked/replayed event sequences. Ported verbatim from
 * agenthud-agui-a2ui's src/agent/contract.ts — the SUPERSET contract (includes
 * DataModelUpdateMessageSchema for two-way data-bound bindings and TreeChoiceSchema.action,
 * both absent from ldnmxx-hack's narrower A2UI subset). Any app consuming this kit — whether
 * it emits ldnmxx-shaped batches (beginRendering + surfaceUpdate only) or agenthud-shaped
 * batches (+ dataModelUpdate) — validates against the SAME schema here.
 *
 ...
 * INTERNAL — a generic pre-baked recording JSON shape (event log + optional decision tree),
 *   useful for any app that replays a scripted demo.
 */
 ...
 * boundary with a clear error, instead of throwing mid-render and blanking the surface.
...
// the tab — and a downstream try/catch only catches *thrown* errors, not a hang. Detect cycles
